...
const HexaNlp = require('../index.js');
...
nock(endpoint)
  ...
  .reply((uri, requestBody) => {
    if ('hexanlp_nlu_data' in requestBody) {
      return [
        200,
...
describe('HexaNlp', function () {
  describe('Instantiation', function () {
    it('Should create an instance of HexaNlp', function () {
      expect(new HexaNlp(endpoint, project, token)).to.be.a(HexaNlp);
    });
    it('Should contain doRequest method', function () {
      expect(new HexaNlp(endpoint, project, token)).to.have.property('request');
    });
    it('Must not hold or show the token', function () {
      const hexanlp = new HexaNlp(endpoint, project, token);
      for (var key in hexanlp) {
        expect(hexanlp[key]).to.not.be.equal(token);
      }
    });
  });
  ...
  //     return hexanlp.get('undefined_path').catch((e) => {
  //       expect(e).to.be.ok();
  //     });
  //   });
  // });
  ...
  //     return hexanlp.delete('undefined_model').catch((e) => {
  //       expect(e).to.be.ok();
  //     });
  //   });
  ...
  //       .train(trainingData)
  //       .then((res) => {
  //         return hexanlp.delete(res.model).then((res) => {
  //           expect(res).to.be.ok();
  //         });
  //       })
  //       .catch((e) => {
