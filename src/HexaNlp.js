...
class HexaNlp {
  ...
  train(hexanlp_nlu_data) {
    let body = { hexanlp_nlu_data };
    return this.send('train', body);
  }
  evaluate(hexanlp_nlu_data) {
    let body = { hexanlp_nlu_data };
  ...
  get(info) {
    ...
    return Promise.reject(
      new Error('Endpoint path is not defined in HexaNlp client : /' + info)
    );
  }
  delete(model) {
    let body = {
